...
local redzlib = {
	Themes = { -- Themes: Darker, Dark, Purple, Green [NEW], Orange [NEW].
    	Darker = {
...
local function GetColor(Instance)
    ...
    return nil  -- ← return nil (stupid return "" fixed)
end
...
local function UpdateAllElements()
    table.foreach(redzlib.Instances, function(_,Val)
        ...
        if not colorProperty then return end  -- ← update
...
function redzlib:GetTheme()
    return redzlib.Save.Theme or "Darker"
end
...
function redzlib:GetScale()
    local currentScale = ScreenGui.Scale.Scale
    local originalValue = ViewportSize.Y / currentScale
    
    return math.floor(originalValue + 0.5)
end
...
function redzlib:MakeWindow(Configs)
	...
	function Window:SetTransparency(transparency)
        if type(transparency) ~= "number" then
            warn("Window:SetTransparency - Transparency must be a number. Got '" .. type(transparency) .. "' instead.")
            return
        end
    
        if transparency < 0 or transparency > 1 then
            warn("Window:SetTransparency - Transparency must be between 0 and 1. Got " .. tostring(transparency) .. ".")
            return
        end
    
        local frame = MainFrame
        if not frame or not frame:IsA("GuiObject") then
            warn("Window:SetTransparency - Target frame is invalid or doesn't exist.")
            return
        end
    ...
    function Window:GetTransparency()
        ...
        return frame.BackgroundTransparency
    end
	function Window:CloseBtn()
	    -- Fixed Window:CloseBtn by @Yuna-ux
		local Dialog = Window:Dialog({
			Title = "Close",
			Text = "You Want Close Ui?",
			Options = {
				{"Confirm", function()
				    if ScreenGui and ScreenGui:IsA("GuiBase") then
                        ScreenGui:Destroy()
                    end
				end},
				{"Cancel"}
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:Enable()
		    if type(Tabs) == "function" then
			    ...
			    warn("Redzlib: 'Tabs' must be a function")
			end
		end
		-- Fixed Tab:Visible by @Yuna-ux
		...
		function Tab:AddToggle(Configs)
            ...
            function Toggle:Callback(...) Funcs:InsertCallback(Callback, ...) end -- removed callback call '()'
		...
		function Tab:AddDropdown(Configs)
			...
			local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
				...
				AddNewOptions = function(List, Clear)
					...
					-- removed callback call CallbackSelected()
					UpdateSelected()
				end
			...
			function Dropdown:Callback(...) Funcs:InsertCallback(Callback, ...) end -- removed callback call '(Select)'
